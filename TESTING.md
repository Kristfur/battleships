...
# Input Validation
Every function was tested to ensure they function correctly with the correct values. Sometimes users can enter incorrect values. Inpute validation ensures the user enters correct values, so the program functions can recieve acceptable values.

All inputs are automatically put into lowercase using the method .lower() for easily indetifying the intended input.
...
| |2|Type 'n'|Game procedes to select board size question|As expected|Pass
| |3|Type 'y'|Game procedes to show the rules|As expected|Pass
...
|Description|Step|Step Description|Expected Result|Actual Result|Pass\Fail
|-|-|-|-|-|-
|Input is checked to ensure it is a valid input|1|Run game, and naviagte to the select board size question
| |2|Type 's'|Game procedes to place ship section with a small board|As expected|Pass
| |3|Type 'm'|Game procedes to place ship section with a medium board|As expected|Pass
| |4|Type 'l'|Game procedes to place ship section with a large board|As expected|Pass
...
|Description|Step|Step Description|Expected Result|Actual Result|Pass\Fail
|-|-|-|-|-|-
|Input is checked to ensure it is a valid input|1|Run game, and naviagte to the place ship section
| |2|Type x : '0' y : '0'|Game procedes to ask for ship direction|As expected|Pass
...
| |3|Type 'e'|Game procedes to place ship|As expected|Pass
| |4|Type 's'|Game procedes to place ship|As expected|Pass
...
| |3|Type 'y'|Game adds the scores to the counter, and procedes to the select board size question|As expected|Pass
| |4|Type 'hello'|Informs user that their answer is invalid and prompts them to try again|As expected|Pass
...
- If all the neighboring cells of all of the reveald hit marks on the board are misses, guess randomly
...
This results in the computer's guesses clustering around hit marks, mimicking what a humand would do when the reveal a hit. This enhances the gaming experience as it makes for a more challenging opponent.
